...
\begin{document}
    ...
    \chapter{Card Types}
    \label{types}
    \section{Cartas de tesoros}
    \cardfigv{assets/treasure.png}
    Las cartas de tesoros se encuentran en el mazo de tesoros. Mientras estén \textbf{en juego} (ver \textbf{Zonas de juego}), una carta de tesoros es una entidad referida como objeto. Esto significa que los objeto pueden ser controlados por jugadores o estar en la tienda.
    
    Los jugadores adquieren objetos durante el juego. Los objetos tienen una amplia variedad de habilidades que van desde modificar las normas de juego a interaccionar con otros jugadores y con monstruos (ver \textbf{Habilidades}\footnote{page~\pageref{abilities}}). Los jugadores deben colocar los objetos que controlan boca arriba delante de ellos.
  
    Cada vez que una carta ordene a un jugador que \textbf{obtiene tesoros}, roban tantas cartas como se indique del mazo de tesoros y las colocan en juego bajo su control.
  
    El \textbf{objeto inicial} de un jugador se considera un \textbf{objeto}. Los objetos iniciales tienen intrínsecamente la cualidad de objeto inicial.
